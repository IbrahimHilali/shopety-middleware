...
import { CreatePayment, Payment, ZainCashConfig } from './payment.interface';
...
export class PaymentService {
  ...
  public async getZainCashConfig(): Promise<ZainCashConfig | null> {
    const zaincashConfig = await this.findByType('zaincash');
    if (!zaincashConfig) {
      return null;
    }
    return zaincashConfig.config as ZainCashConfig;
  }
}
